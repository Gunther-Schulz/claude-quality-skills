...
echo "Uninstalling claude-quality-skills..."
echo ""

# Uninstall plugin via CLI
if command -v claude &>/dev/null; then
    claude plugin uninstall quality-skills@local 2>/dev/null && echo -e "  ${GREEN}uninstalled${NC}  quality-skills@local" || true
    ...
    echo "  /plugin uninstall quality-skills@local"
...
if command -v jq &>/dev/null && [ -f "$SETTINGS" ]; then
    jq '
      del(.enabledPlugins["quality-skills@local"]) |
...
echo "  rm -f ~/.claude/quality-skills.local.md"
echo "  rm -rf \${XDG_STATE_HOME:-\$HOME/.local/state}/claude-quality-skills"
echo "  rm -rf \${XDG_CONFIG_HOME:-\$HOME/.config}/claude-quality-skills  # old config location"
